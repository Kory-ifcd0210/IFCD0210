...
PHXSBridge = {
		...
		try {
			...
			if (welcomeDesc !== undefined) {
				...
				if (welcomeDesc.count) {
					...
					w.accountStatus       = (welcomeDesc.getInteger(kentryStatusStr) === 1) ? 'trial' : 'paid';
					w.accountType         = welcomeDesc.getString(kaccountTypeStr);
					w.secondsLeftInTrial  = (w.accountStatus === 'paid') ? 0 : welcomeDesc.getLargeInteger(kleftStr);
	 ...
	 */
	getRecentFiles: function(dontStringify) {
		/**
	...
	getCCXUserJSON : function(mode) {
		...
		if (isFNFT) {
			...
			supportJSONBlocks.recentFiles = this.getRecentFiles(true);
		}
...
CCXHostBridge = {
	...
	getUserJSONData: function(hostID) {
		...
		switch(hostID) {
			case 'PHXS':
				userData = PHXSBridge.getCCXUserJSON();  // because PS has to be different
				break;
